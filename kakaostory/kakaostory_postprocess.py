...
INPUT_PATH = Path("kakaostory_popup_posts.json")
OUTPUT_PATH = INPUT_PATH
LOG_PATH = Path("kakaostory_postprocess.log")
TARGET_P_NUM = -1  # 특정 p_num을 테스트하려면 양수로 설정
TEST_LIMIT = 0  # 샘플 테스트: 20같은 양수로 설정 / 전체 데이터: 0으로 설정
FORCE_REPROCESS = False
MIN_CAPTION_LENGTH = 20
USER_NUM_REGEX = r"(?:추천(?:아이디|인)?\s*)?((?:[678]\d{6})|\d{8})"
REQUEST_TIMEOUT = 30
EASYOCR_LANGS = ["ko", "en"]
...
def extract_user_number(content: str, media_caption: str, name: str) -> Optional[str]:
    ...
    match = USER_NUM_PATTERN.search(combined)
    if not match:
        return None
...
def process_target_post(post: dict, all_posts: List[dict]) -> bool:
    ...
    # user_id로 기존 user_num 찾기 (속도 최적화)
    ...
    # user_num 추출 시도
    user_num = extract_user_number(
        post.get("content", ""),
        chosen_caption or existing_caption,
        post.get("name", ""),
    )
    if user_num and user_num != existing_user_num:
        post["user_num"] = user_num
        updated = True
        logger.info("  → user_num 추출됨: %s", user_num)
    elif existing_user_num and not user_num:
...
def process_posts(posts: List[dict], targets: Iterable[dict]) -> Tuple[int, int]:
    updated_posts = 0
    propagated_total = 0
    index = 0
    ...
    return updated_posts, propagated_total


def save_posts(posts: List[dict], updated_posts: int, propagated_total: int) -> None:
    """게시물 데이터를 파일에 저장"""
    OUTPUT_PATH.write_text(json.dumps(posts, ensure_ascii=False, indent=2), encoding="utf-8")
    logger.info(
        "JSON 업데이트 완료 (media_caption/user_num 갱신 %d건, 동일 이름 전파 %d건)",
        updated_posts,
        propagated_total,
    )
...
def main() -> None:
    ...
    try:
        updated_posts, propagated_total = process_posts(posts, targets)
        ...
        if has_changes or updated_posts > 0 or propagated_total > 0:
            # 실제 변경사항이 있거나 카운터에 변경이 있으면 무조건 저장
            save_posts(posts, updated_posts, propagated_total)
            logger.info("✅ 중간 저장 완료 (처리된 데이터 보존됨)")
        else:
            logger.info("변경된 내용이 없어 저장하지 않습니다.")
        raise

    if updated_posts or propagated_total:
        save_posts(posts, updated_posts, propagated_total)
    else:
    ...
    if targets:
        ...
        logger.info("첫 게시물 user_num: %s", sample_post.get("user_num"))
